...
def run_command(base_cmd, args_list) -> Tuple[int, str, str]:
    """
    Run a whitelisted command (or action handlers like calc, lock).
    ...
    try:
        ...
        if base_cmd == "shutdown":
            # args_list expected like ['/s', '/t', '0'] or similar
...
def run_raw_command(command_text) -> Tuple[int, str, str]:
    """
    Run an arbitrary raw command (used only when --allow-arbitrary and confirmed).
    """
    try:
